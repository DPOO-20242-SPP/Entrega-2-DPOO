...
public class MainCliente {
    ...
    private static void registrarCliente() {
        ...
        String hashCompat = Integer.toHexString(pwd.hashCode()); // hash sencillito

        Cliente nuevo = new Cliente(id, nombre, correo, login, hashCompat);

        usuariosByCorreo.put(correo, nuevo);
        usuariosByLogin.put(login, nuevo);

        System.out.println("Registro exitoso: " + nombre + " [Cliente]");
        snapshot();
    }

    private static Cliente loginCliente() {
        sysoLines("", "=========== LOGIN CLIENTE ===========");
        ...
        if (!(u instanceof Cliente c)) {
            System.out.println("Este ejecutable es solo para CLIENTES.");
            return null;
        }
        System.out.println("Bienvenid@ " + c.getName() + " (Cliente)");
        return c;
    }
    ...
    private static void listarEventosDetallado() {
        ...
        for (int i = 0; i < eventos.size(); i++) {
            ...
            System.out.println("[" + i + "] " + e.getNombre() + " (" + e.getTipo() + ") en " +
                               e.getVenue().getNombre() + " — cancelado:" + e.isCancelado());
            List<Localidad> locs = e.getLocalidades();
            for (int j = 0; j < locs.size(); j++) {
                Localidad l = locs.get(j);
                System.out.println("   - (" + j + ") " + l.getNombre() + " $" +
                                   l.getPrecioPublico() + " cap=" + l.getCapacidad());
            }
        }
    }
    ...
    private static void flujoCompra(Cliente c) {
        ...
        if (idxL < 0 || idxL >= e.getLocalidades().size()) {
            System.out.println("Índice inválido");
            return;
        }
        ...
        if (qty > l.getCapacidad()) {
            System.out.println("No hay cupo suficiente");
            return;
        }
    ...
    private static void flujoTransferencia(Cliente c) {
        ...
        for (Tiquete t : tiquetes) {
            if (t.getPropietarioActual() == c) mios.add(t);
        }
        if (mios.isEmpty()) {
        ...
        for (int i = 0; i < mios.size(); i++) {
            ...
            System.out.println("(" + i + ") " + t.getId() + " ev=" + t.getEvento().getNombre() +
        ...
        if (!(dest instanceof Cliente)) {
            System.out.println("Destino debe ser Cliente existente");
            return;
        }
    ...
    private static void flujoReembolsoCalamidad(Cliente c) {
        ...
        Reembolso r = new Reembolso(
            ...
            t,
            t.getPrecioBase(),
            ADMIN != null ? ADMIN.getCostoEmisionFijo() : 0.0,
    ...
    private static void flujoHacerContraoferta(Cliente comprador) {
        ...
        System.out.println("Contraoferta registrada. El vendedor deberá responder (flujo futuro).");
    }
